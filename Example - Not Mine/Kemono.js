...
// @icon         https://www.google.com/s2/favicons?sz=32&domain=patreon.com
// @grant        none
// @license      MIT
// ==/UserScript==
...
function GetCreatorId() {
	if (typeof __next_f !== 'undefined') {
        return __next_f.toString().match(/"creator":{"data":{"id":"(\d+)","type":"user"}/)[1]
    }
    if (typeof __NEXT_DATA__ !== 'undefined') {
        return __NEXT_DATA__.innerHTML.match(/"creator":{"data":{"id":"(\d+)","type":"user"}/)[1]
    }

    console.error('Unable to get creator ID');
	return null;
}

window.addEventListener("load", () => {
    window.location.href = `https://kemono.cr/patreon/user/${GetCreatorId}`
})
