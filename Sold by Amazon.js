// ==UserScript==
// @name        Sold by Amazon
// @description Sold by Amazon
// @version     1.2
// @author      Octihex
// @icon        https://www.google.com/s2/favicons?sz=32&domain=amazon.fr
// @match       *://*.amazon.fr/*
// @match       *://*.amazon.de/*
// ==/UserScript==
'use strict'

let searchDiv = document.querySelector("div[id=nav-search]")
let site = window.location.hostname
let SoldByAmazonCode = ''

if (site == "www.amazon.fr"){
    SoldByAmazonCode = "A1X6FK5RDHNB96"
}

if (site == "www.amazon.de"){
    SoldByAmazonCode = "A3JWKAKR8XB7XF"
}

document.querySelector("form[id=nav-search-bar-form]").remove() // Delete the original searchbar

searchDiv.appendChild(
    Object.assign(document.createElement('input'), {
        id: 'searchzone',
        placeholder: "Rechercher Amazon.fr",
        style: "width: 100%; margin-right: 0px; border: 0px; border-radius: 5px 0px 0px 5px; z-index: 3;"
    })
)

searchDiv.appendChild(
    Object.assign(document.createElement('input'), {
        id: 'searchbutton',
        type: 'image',
        src: "data:image/png;base64,",
        style: 'margin-left: 0px;',
        onclick: filterSearch
    })
)

let zone = document.getElementById("searchzone")
let button = document.getElementById("searchbutton")

//button.onclick = filterSearch
zone.value = document.querySelector('div[id=search] > span[class=rush-component]').querySelector('h2 > span[class]').innerText.replaceAll('"', '') || ''
zone.addEventListener('keydown', function (e) {
    if (e.key === 'Enter') {
    	filterSearch()
    }
});

function filterSearch() {
    window.location.href = `https://${site}/s?k=${encodeURIComponent(zone.value)}&emi=${SoldByAmazonCode}`
}

let itemsList = Array.from(document.querySelectorAll('div[role=listitem]'))
let searchText = document.querySelector('span[class="a-color-state a-text-bold"]').textContent.toLocaleLowerCase().replaceAll('"', "")

if (itemsList.length > 0) {
    for (let element of itemsList) {
        if (!element.querySelector('h2').textContent.toLocaleLowerCase().includes(searchText)) {
            element.remove()
        }
    }
}
